...
```
There are 596 rows and 32 columns in the cancer_sample dataset.There are 2 different data types, which are chr and dbl respectively.
...
glimpse(flow_sample)
```
There are 218 rows and 7 columns in the flow_sample dataset.There are 2 different data types, which are chr and dbl respectively.
...
```
There are 10032 rows and 22 columns in the parking_meters dataset. There are 2 different data types, which are chr and dbl respectively. 
...
```
There are 146611 rows and 20 columns in the vancouver_trees dataset. There are 3 different data types, which are chr, date and dbl respectively. 
...
<!-------------------------- Start your work below ---------------------------->
I've decided to opt for the **'flow_sample'** dataset primarily because of its manageable size, striking a balance that is neither too large nor too small for thorough analysis. 
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
Delving into this dataset holds the promise of uncovering valuable insights into hydrological patterns, extreme flow events, and various facets of water flow data, all of which may find practical applications in environmental studies and water resource management.
<!----------------------------------------------------------------------------->
...
The select() function is employed to specify which columns (variables) to retain in the new tibble, while excluding others. In this case, columns such as 'station_id,' 'sym,' 'year,' 'month,' and 'day' are removed, considering that 'date' has already been included.
